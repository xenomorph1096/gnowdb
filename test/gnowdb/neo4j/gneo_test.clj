...
(deftest createClass-test
  ...
  (testing "Error in Returning a query (execute? false)"
    (is 
      (= (select-keys {:query "MERGE (node:Class { className:{className}, classType:{classType}, isAbstract:{isAbstract}, UUID:{UUID} } )", :parameters {"className" "t-db24", "classType" "NODE", "isAbstract" true, "UUID" "8f1d44e1-a3c1-45aa-a428-4b3dd00f6eb3"}} [:query]) 
